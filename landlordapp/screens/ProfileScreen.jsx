...
import { LogOut, User, Phone, Mail, Lock, ChevronRight } from 'lucide-react-native'
import { useUser } from '../context/UserContext'
import api from '../utils/api'

const ProfileScreen = () => {
    const { user, logout, updateProfile, refreshProfile } = useUser()
    ...
    const handlePasswordChange = async () => {
        if (!currentPw || !newPw) return Alert.alert('Error', 'Fill in both password fields')
    ...
    return (
        <SafeAreaView className="flex-1 bg-white">
            <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}
                refreshControl={<RefreshControl refreshing={pwLoading} onRefresh={onRefresh} tintColor="#1d4ed8" />}
            >
                ...
                <View className="bg-slate-50 rounded-2xl p-4 mb-4">
                    <View className="flex-row items-center justify-between mb-3">
                        ...
                        <TouchableOpacity onPress={() => {
                            ...
                            <Text className="text-blue-700 text-sm font-medium">{editMode ? 'Cancel' : 'Edit'}</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    {editMode && (
                        <TouchableOpacity
                            onPress={handleSave}
                            disabled={saving}
                            className="bg-blue-700 py-3 rounded-xl mt-4 items-center"
                        >
                            {saving
